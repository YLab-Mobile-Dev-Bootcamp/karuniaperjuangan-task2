...
Future<List<String>> getKotaList() async {
  ...
  final response = await Dio().get("https://api.rajaongkir.com/starter/city",
      options: Options(
          headers: {"key": "237d4652a598ea4bd9ac7047bba33a90"},
  ...
  if (response.statusCode == 200) {
    ...
    cityList.forEach((city) {
      returnList.add(city["city_name"].toString());
    });
  }
  return returnList;
}
...
Future<List<String>> GetLayananList(
    {required int idAsal,
    required int idTujuan,
    required int berat,
    required String kurir}) async {
  List<String> returnList = [];
  final response = await Dio().post("https://api.rajaongkir.com/starter/cost",
      data: {
        'origin': '$idAsal',
